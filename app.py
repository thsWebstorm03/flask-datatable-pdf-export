...
def generate_pdf():
   # Execute your Python script when the button is clicked
   #  subprocess.run(['python', 'pyexcel.py'])  # Replace 'python' with your Python interpreter command
   output = io.BytesIO()
   ...
   for row_num, (no, name, position, office, facebook_url, instagram_url, twitter_url, linkedin_url, tiktok_url, youtube_url,startdate, salary ) in enumerate(data, start=2):
      ...
      image_path = './static/images/twitter.png'  # Replace with the actual path to your image
